...
public partial class ChangeManagement_frmAddChangeType : System.Web.UI.Page
{
    ...
    private void FillRequestTypeDetails()
    {
        ...
        try
        {
            ...
            DataTable SD_ReqType = new FillSDFields().FillChangeType();
    ...
    protected void SaveData()
    {
        try
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SD_spChangeType", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@ChangeTypeRef ", txtChangeType.Text);
                    cmd.Parameters.AddWithValue("@ChangeTypeDef", txtReqDescription.Text);
        ...
        $"if (window.location.pathname.endsWith('/frmAddChangeType.aspx')) {{ success_noti('{HttpUtility.JavaScriptStringEncode("Saved Successfully!")}'); setTimeout(function() {{ window.location.reload(); }}, 2000); }}", true);
                    }
            ...
            if (e.CommandName == "DeleteEx")
            {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SD_spChangeType", con))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@ChangeTypeRef", ReqTypeRef);
                        cmd.Parameters.AddWithValue("@ChangeTypeDef", ReqTypeDef);
                        cmd.Parameters.AddWithValue("@OrgRef", ddlOrg.SelectedValue.ToString());
                        ...
                        if (res > 0)
                        {
                            ...
                            ScriptManager.RegisterStartupScript(this, GetType(), "showNotification",
         $"if (window.location.pathname.endsWith('/frmAddChangeType.aspx')) {{ success_noti('{HttpUtility.JavaScriptStringEncode("Deleted Successfully!")}'); setTimeout(function() {{ window.location.reload(); }}, 2000); }}", true);
                        }
            ...
            if (e.CommandName == "SelectState")
            {
                ...
                //Get the value of column from the DataKeys using the RowIndex.
                ID = Convert.ToInt32(gvReqType.DataKeys[rowIndex].Values["ID"]);
                txtChangeType.Text = gvReqType.Rows[rowIndex].Cells[1].Text;
        ...
        try
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SD_spChangeType", con))
                {
                    ...
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "showNotification",
        $"if (window.location.pathname.endsWith('/frmAddChangeType.aspx')) {{ success_noti('{HttpUtility.JavaScriptStringEncode("Updtaed Successfully!")}'); setTimeout(function() {{ window.location.reload(); }}, 2000); }}", true);
                    }
